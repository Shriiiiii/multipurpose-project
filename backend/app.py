...
from code_checker import check_code # FIX: Corrected import
...
# FIX: Removed the incorrect check_code_similarity function definition
...
def generate_text_report(result_data, output_path="report.pdf"):
    ...
    elements.append(Spacer(1, 12))
    elements.append(Paragraph(f"Overall Similarity: <b>{result_data.get('similarity', 0)}%</b>", styles["Normal"]))
    elements.append(Paragraph(f"AI Detection: <b>{result_data.get('ai', 0)}%</b>", styles["Normal"]))
    elements.append(Paragraph(f"Human Detection: <b>{result_data.get('human', 0)}%</b>", styles["Normal"]))
    elements.append(Spacer(1, 12))
...
def api_check_text():
    ...
    # Generate report using the actual result
    report_path = os.path.join(app.root_path, "report.pdf")
    generate_text_report(result_data=result, output_path=report_path)

    
    source_urls = [s.get('url', 'N/A') for s in result.get('sources', [])]
    save_report('text', text, result.get('similarity', 0), source_urls,
                result.get('highlighted_text', ''), result.get('explanation', []))

    # Return the actual result from check_plagiarism
...
def api_compare_pdfs():
    ...
    # FIX: Save meaningful text and report details
    save_report('pdf', f"PDF Compare (Sim: {result['similarity']}%)", result['similarity'], [], "", []) 
...
def api_check_code():
    ...
    # FIX: Call the single-argument check_code twice and take the maximum similarity
    result1 = check_code(code1)
    result2 = check_code(code2)
    
    # Combine the best result (highest similarity)
    if result1['similarity'] > result2['similarity']:
        result = result1
    else:
        result = result2

    save_report('code', f"Code Compare: {code1[:50]}...", result.get('similarity', 0), [], result.get('highlighted_code', ''), [])
    return jsonify(result)
